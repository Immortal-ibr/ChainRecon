...
from chainrecon.analysis import FirmwareAnalyzer, ReportGenerator, ScannerAnalyzer, SSLAnalyzer, TrafficAnalyzer
from chainrecon.utils.artifacts import safe_token
from chainrecon.utils.logging_config import get_logger, setup_logging
...
def handle_network_config(args) -> int:
    ...
    from chainrecon.utils.config import get_network_config, save_network_config, reset_config
    from chainrecon.utils.network import list_interfaces
...
def handle_scan(args) -> int:
    from chainrecon.runners import NmapRunner
...
def handle_capture(args) -> int:
    from chainrecon.runners import CaptureRunner
    from chainrecon.runners.base import ToolNotFoundError
...
def main(argv: List[str] | None = None) -> int:
    ...
    if args.command is None or args.command == "tui":
        from chainrecon.tui.app import run_tui
        run_tui()
        return 0

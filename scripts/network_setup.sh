#!/bin/bash
# ==========================================
# ChainRecon - Network Setup Script
# ==========================================
# Configures IP forwarding, NAT, and iptables
# for IoT traffic interception via a physical router.
...
if [ "$EUID" -ne 0 ]; then
    echo -e "${RED}[!] Please run as root (sudo bash network_setup.sh)${NC}"
    exit 1
fi

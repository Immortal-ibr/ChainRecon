...
from chainrecon.utils import platform_info


class OSDetectionTests(unittest.TestCase):
    @patch("chainrecon.utils.platform_info.platform.system", return_value="Windows")
    def test_get_os_windows(self, _):
        self.assertEqual(platform_info.get_os(), "windows")

    @patch("chainrecon.utils.platform_info.platform.system", return_value="Linux")
    def test_get_os_linux(self, _):
        self.assertEqual(platform_info.get_os(), "linux")

    @patch("chainrecon.utils.platform_info.platform.system", return_value="Darwin")
    def test_get_os_darwin(self, _):
        self.assertEqual(platform_info.get_os(), "darwin")

    @patch("chainrecon.utils.platform_info.get_os", return_value="windows")
    def test_is_windows_true(self, _):
        self.assertTrue(platform_info.is_windows())

    @patch("chainrecon.utils.platform_info.get_os", return_value="linux")
    ...
    @patch("chainrecon.utils.platform_info.get_os", return_value="darwin")
...
class FindToolTests(unittest.TestCase):
    ...
    @patch("chainrecon.utils.platform_info.is_windows", return_value=False)
    def test_not_found_returns_none(self, *_):
        self.assertIsNone(platform_info.find_tool("nmap"))

    @patch("chainrecon.utils.platform_info.is_windows", return_value=True)
    @patch("chainrecon.utils.platform_info.shutil.which")
    ...
    @patch("chainrecon.utils.platform_info.is_windows", return_value=True)
    @patch("chainrecon.utils.platform_info.shutil.which", return_value=None)
    @patch("chainrecon.utils.platform_info.os.path.isdir", return_value=True)
    @patch("chainrecon.utils.platform_info.os.path.isfile")
    ...
    @patch("chainrecon.utils.platform_info.is_windows", return_value=False)
    @patch("chainrecon.utils.platform_info.os.path.isdir", return_value=True)
    @patch("chainrecon.utils.platform_info.os.path.isfile", return_value=True)
...
class RequireToolTests(unittest.TestCase):
    @patch("chainrecon.utils.platform_info.find_tool", return_value="/usr/bin/nmap")
...
class CheckAllToolsTests(unittest.TestCase):
    @patch("chainrecon.utils.platform_info.find_tool")
